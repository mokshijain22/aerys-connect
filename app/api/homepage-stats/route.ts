...
export async function GET() {
  try {
    ...
    const [
      [[totalVehicles]], [[activeJobs]], [[pendingJobs]], [[slaBreached]],
      [[completedToday]], [[jobsToday]], [[deliveriesToday]], [statusRows], [[pendingClaims]],
      [recentVehicles], [recentJobs], [recentClaims],
      ...
      pool.query(`SELECT status, COUNT(*) AS total FROM job_cards WHERE deleted_at IS NULL${jobFilter} GROUP BY status`, jParams),
      pool.query(`SELECT COUNT(*) AS total FROM warranty_claims wc JOIN job_cards jc ON wc.job_card_id = jc.job_card_id WHERE wc.status IN ('submitted','dealer_approved') AND wc.deleted_at IS NULL AND jc.deleted_at IS NULL${claimScope}`, claimParams),
      pool.query(`SELECT 'vehicle' AS type, chassis_number AS ref, created_at AS ts FROM vehicles WHERE deleted_at IS NULL${vehicleFilter} ORDER BY created_at DESC LIMIT 5`, vParams),
      pool.query(`SELECT 'job_card' AS type, job_card_id AS ref, registered_at AS ts FROM job_cards WHERE deleted_at IS NULL${jobFilter} ORDER BY registered_at DESC LIMIT 5`, jParams),
      pool.query(`SELECT 'warranty_claim' AS type, wc.claim_number AS ref, wc.submitted_at AS ts FROM warranty_claims wc JOIN job_cards jc ON wc.job_card_id = jc.job_card_id WHERE wc.deleted_at IS NULL AND jc.deleted_at IS NULL${claimScope} ORDER BY wc.submitted_at DESC LIMIT 5`, claimParams),
    ]);
